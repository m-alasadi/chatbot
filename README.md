...
export type AllowedToolName = 'search_entries' | 'get_entry_by_id' | ...
```
...
export const TOOL_SEARCH_PROJECTS: ChatCompletionTool = {
  ...
  function: {
    name: "search_entries",
    description: "البحث العميق في عناصر البيانات من المصدر المهيأ...",
    parameters: {
      type: "object",
      properties: {
        query: {
...
|--------|-------|-----------|
| `search_entries` | البحث في العناصر | `query`, `section?`, `limit?` |
| `get_entry_by_id` | تفاصيل عنصر محدد | `id` |
| `list_entry_categories` | قائمة الفئات | `include_counts?` |
| `get_latest_entries` | أحدث العناصر | `limit?`, `section?` |
| `get_entry_statistics` | إحصائيات عامة | - |
...
User: "أريد مشاريع طبية"
  ↓
GPT: يقرر استدعاء search_entries
  ↓
Function Call: search_entries({ query: "طبية", section: "المشاريع الطبية", limit: 5 })
  ↓
